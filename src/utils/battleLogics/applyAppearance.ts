  ...
  for (const effect of ability.appear) {
    switch (effect) {
      case "weather_change":
        ...
        if (ability.name === '잔비') {
          setWeather("비");
          logs.push(`${pokemon.base.name}의 특성으로 날씨가 비가 되었다!`);
        }
        if (ability.name === '눈퍼뜨리기') {
          setWeather("싸라기눈");
          logs.push(`${pokemon.base.name}의 특성으로 날씨가 싸라기눈이 되었다!`);
        }
        if (ability.name === '모래날림') {
          setWeather("모래바람");
          logs.push(`${pokemon.base.name}의 특성으로 날씨가 모래바람이 되었다!`);
        }
        break;
      case "field_change":
        if (ability.name === '일렉트릭메이커') {
          setField("일렉트릭필드");
          logs.push(`${pokemon.base.name}의 특성으로 필드가 일렉트릭필드로 바뀌었다!`);
        }
        if (ability.name === '그래스메이커') {
          setField("그래스필드");
          logs.push(`${pokemon.base.name}의 특성으로 필드가 그래스필드로 바뀌었다!`);
        }
        if (ability.name === '미스트메이커') {
          setField("미스트필드");
          logs.push(`${pokemon.base.name}의 특성으로 필드가 미스트필드로 바뀌었다!`);
        }
        if (ability.name === '사이코메이커') {
          setField("사이코필드");
          logs.push(`${pokemon.base.name}의 특성으로 필드가 사이코필드로 바뀌었다!`);
        }
        break;
      ...
      case "aura_change":
        if (ability.name === '페어리오라') {
          setAura("페어리오라");
          logs.push(`${pokemon.base.name}의 특성으로 페어리오라가 생겼다!`);
        } else {
          setAura("다크오라");
          logs.push(`${pokemon.base.name}의 특성으로 다크오라가 생겼다!`);
        }
        break;
      ...
      case "disaster":
        if (ability.name === '재앙의구슬') {
          addDisaster("재앙의구슬");
          logs.push(`${pokemon.base.name}의 특성으로 ${ability.name} 효과가 발동했다!`);
        }
      ...
      // 아래 항목은 필요 시 확장
      case "form_change":
        logs.push(`${pokemon.base.name}의 폼이 변화했다!`);
        break;

      case "rank_change":
        if (ability.name === '위협') {
          const updatedOpponent = changeRank(enemyPokemon, "attack", -1);
          updatePokemon("enemy", activeEnemy, updatedOpponent);
      ...
      case "heal":
        logs.push(`${pokemon.base.name}이 회복 효과를 발동했다!`);
        break;

      case "ability_change":
        logs.push(`${pokemon.base.name}의 특성이 변화했다!`);
        break;
    }
  }
